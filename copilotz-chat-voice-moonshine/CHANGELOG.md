# @copilotz/chat-voice-moonshine

## 0.75.0 — 2026-09-16

Synchronize the voice package release with Copilotz 0.75.0 and the workspace
package set.

## 0.70.1 — 2026-09-09

Synchronize the frontend package suite with Copilotz 0.70.1.

## 0.70.0 — 2026-09-08

Synchronize the frontend package suite with Copilotz 0.70.0.

## 0.69.1 — 2026-09-07

Synchronize the frontend package suite with Copilotz 0.69.1.

## 0.68.0 — 2026-09-07

Synchronize the frontend package suite with Copilotz 0.68.0 Agent connections.

## 0.66.0 — 2026-09-05

Synchronize peer dependencies with chat-ui 0.66.0.

## 0.65.1

### Patch Changes

- Synchronize the frontend package suite with Copilotz tool-result handling.

## 0.65.0

### Minor Changes

- Synchronize the frontend package suite with Copilotz 0.65.0.

## 0.64.2

### Patch Changes

- Synchronize the frontend package suite with Copilotz 0.64.2.

## 0.64.1

### Patch Changes

- Synchronize the frontend package suite with Copilotz 0.64.1.

## 0.64.0

### Minor Changes

- Synchronize the frontend package suite with Copilotz 0.64.0.

## 0.59.24

### Patch Changes

- Align the voice package release with Copilotz and the frontend package set.

## 0.59.22

### Patch Changes

- Align the voice package release with Copilotz and the frontend package set.

## 0.59.21

### Patch Changes

- Align the voice package release with Copilotz and the frontend package set.

## 0.59.20

### Patch Changes

- Align the voice package release with Copilotz and the frontend package set.
