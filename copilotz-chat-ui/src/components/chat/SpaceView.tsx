    ...
    <div className="divide-y rounded-lg border">
      {items.map((thread) => (
        <button
          type="button"
          key={thread.id}
          className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
...
export const SpaceView: React.FC<SpaceViewProps> = ({
  ...
  const builtInContent = section?.id === "overview"
    ...
    : section?.id === "conversations"
      ? <CollectionState<ChatThread> collection={data?.conversations} noun="Conversations">{(items) => <ConversationList items={items} onOpen={onOpenConversation} />}</CollectionState>
    : null;
  ...
  return (
    <section className={`flex min-h-0 flex-1 flex-col overflow-hidden ${className}`} aria-label={`${space.name || space.id} Space`}>
      <header className="flex shrink-0 items-center gap-3 border-b px-4 py-3 sm:px-6">
        {onClose && (
          <Button type="button" variant="ghost" size="icon" onClick={onClose} aria-label="Back to conversation">
            <ChevronLeft className="h-4 w-4" aria-hidden="true" />
          </Button>
        )}
        <h1 className="min-w-0 flex-1 truncate text-base font-semibold">{space.name || space.id}</h1>
      </header>
